...
(async () => {
  ...
  app.post("/request-credentials", async (req, res) => {
    try {
      console.log(req.body);
      let rec;
      ...
      const idU = req.body.id;
      if (withPass){
        rec = users[idU] ? users[idU].rec : null;
      }
      else {
      users = await User.findOne({idU});
      rec = users.idU ? toArrayBuffer(users.rec.buffer) : null;
      }
      if (!rec) {
  ...
  app.post("/store-user-record", (req, res) => {
    try {
      ...
      if (!users[idU]) {
        users[idU] = userRec;
        const mongouser = new User({idU:idU,rec:userRec.rec});
        mongouser.save()
        .then(item => {
